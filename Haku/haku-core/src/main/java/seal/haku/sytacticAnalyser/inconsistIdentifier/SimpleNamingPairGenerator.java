...
public class SimpleNamingPairGenerator {
...
//				calMethodDistance(mNodes.get(i).getTokens(), mNodes.get(j)
...
//		}
	...
	private void calFieldDistance(ArrayList<NameToken> tokenA,
		...
		if (lenA == 1 || lenB == 1)
			return;
		// because it is noun
		if ((tokenA.get(lenA - 1).getToken().equals(tokenB.get(lenB - 1)
				.getToken()))) {
	...
	@SuppressWarnings("unchecked")
	private void calMethodDistance(ArrayList<NameToken> tokenA,
			ArrayList<NameToken> tokenB, String nameA, String nameB) {
		// because it is verb
		if ((tokenA.get(0).getToken().equals(tokenB.get(0).getToken()))) {
			ArrayList<NameToken> tokenACopy = (ArrayList<NameToken>) tokenA
					.clone();
			ArrayList<NameToken> tokenBCopy = (ArrayList<NameToken>) tokenB
					.clone();
			tokenACopy.remove(0);
			tokenBCopy.remove(0);
			int sizeA = tokenACopy.size();
			int sizeB = tokenBCopy.size();
			if (sizeA>1 && sizeB>1 && ( tokenACopy.get(sizeA-1).getToken().equals(tokenBCopy.get(sizeB-1).getToken()))) {
				addStringToDictionary(tokenACopy, tokenBCopy, nameA, nameB);
			}
		}
	}
