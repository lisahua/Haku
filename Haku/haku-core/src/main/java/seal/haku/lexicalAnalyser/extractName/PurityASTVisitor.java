...
public class PurityASTVisitor extends PoorNameDetector {
	...
	public PurityASTVisitor() {
		
	}
	
	public PurityASTVisitor(String filePath) {
...
//					// String path, String type, String name,String method,
//					// String usage
//					String method = entry.getKey().replace("~", "");
//					DBEntity.writeUsage(filePath,
//							resolveValueType(method), method,
//							method, usage);
//				}
//			}
//		}
...
//	}
	...
	private void aggregateSingleMethodUsage(String sb) {
		...
		for (String usg : usages) {
			...
			if (!usg.contains(":")) {
				if (usg.endsWith("1")) {
					usg = usg.replace("1", "").trim();
					if (fieldValueTypes.containsKey(usg))
					usedField.add(usg);
				}
...
//			DBEntity.writeUsage(filePath+","+mtd + ","+ fieldString);
	}
	...
	private String parseExpression(Expression exp) {
		...
		if (exp instanceof Name) {
			 if (fieldValueTypes.containsKey(exp.toString()))
			 return exp.toString() + " ";
			 else
			 return resolveValueType(exp.toString());
		}
		if (exp instanceof Assignment) {
			String returnS = parseExpression(((Assignment) exp)
					.getLeftHandSide());
			if (!returnS.equals(""))
				return returnS+"1";
